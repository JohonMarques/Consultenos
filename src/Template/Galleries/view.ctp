            ...
            <div class="box box-solid">
                ...
                <div class="box-body">
                    <dl class="dl-horizontal">
                        ...
                        <td><?= $this->Html->image($gallery->photo, array('width' => '200px')) ?></td>
                        <dt scope="row"><?= __('Photo Dir') ?></dt>
                        <dd><?= h($gallery->photo_dir) ?></dd>
                        <dt scope="row"><?= __('Category') ?></dt>
                        <dd><?= $gallery->has('category') ? $this->Html->link($gallery->category->name, ['controller' => 'Categories', 'action' => 'view', $gallery->category->id]) : '' ?></dd>
                        <dt scope="row"><?= __('Id') ?></dt>
                        <dd><?= $this->Number->format($gallery->id) ?></dd>
                        <dt scope="row"><?= __('Created') ?></dt>
                        <dd><?= h($gallery->created) ?></dd>
                        <dt scope="row"><?= __('Modified') ?></dt>
                        <dd><?= h($gallery->modified) ?></dd>
                    </dl>
                </div>
            </div>
        </div>
